...
class TestMsgEthereumSigntx(TrezorTest):
    def test_ethereum_signtx_known_erc20_token(self):
        ...
        with self.client:
            ...
            assert (
                hexlify(sig_r)
                == b"ec1df922115d256745410fbc2070296756583c8786e4d402a88d4e29ec513fa9"
            )
            assert (
                hexlify(sig_s)
                == b"7001bfe3ba357e4a9f9e0d3a3f8a8962257615a4cf215db93e48b98999fc51b7"
            )
    ...
    def test_ethereum_signtx_unknown_erc20_token(self):
        ...
        with self.client:
            ...
            assert (
                hexlify(sig_r)
                == b"2559bbf1bcb80992b6eaa96f0074b19606d8ea7bf4219e1c9ac64a12855c0cce"
            )
            assert (
                hexlify(sig_s)
                == b"633a74429eb6d3aeec4ed797542236a85daab3cab15e37736b87a45697541d7a"
            )
    ...
    def test_ethereum_signtx_nodata(self):
        ...
        assert (
            hexlify(sig_r)
            == b"2f548f63ddb4cf19b6b9f922da58ff71833b967d590f3b4dcc2a70810338a982"
        )
        assert (
            hexlify(sig_s)
            == b"428d35f0dca963b5196b63e7aa5e0405d8bff77d6aee1202183f1f68dacb4483"
        )
        ...
        assert (
            hexlify(sig_r)
            == b"3bf0470cd7f5ad8d82613199f73deadc55c3c9f32f91b1a21b5ef644144ebd58"
        )
        assert (
            hexlify(sig_s)
            == b"48b3ef1b2502febdf35e9ff4df0ba1fda62f042fad639eb4852a297fc9872ebd"
        )
    ...
    def test_ethereum_signtx_data(self):
        ...
        assert (
            hexlify(sig_r)
            == b"e90f9e3dbfb34861d40d67570cb369049e675c6eebfdda6b08413a2283421b85"
        )
        assert (
            hexlify(sig_s)
            == b"763912b8801f76cbea7792d98123a245514beeab2f3afebb4bab637888e8393a"
        )
        ...
        assert (
            hexlify(sig_r)
            == b"dd96d82d791118a55601dfcede237760d2e9734b76c373ede5362a447c42ac48"
        )
        assert (
            hexlify(sig_s)
            == b"60a77558f28d483d476f9507cd8a6a4bb47b86611aaff95fd5499b9ee9ebe7ee"
        )
    ...
    def test_ethereum_signtx_message(self):
        ...
        assert (
            hexlify(sig_r)
            == b"81af16020d3c6ad820cab2e2b0834fa37f4a9b0c2443f151a4e2f12fe1081b09"
        )
        assert (
            hexlify(sig_s)
            == b"7b34b5d8a43771d493cd9fa0c7b27a9563e2a31799fb9f0c2809539a848b9f47"
        )
    ...
    def test_ethereum_signtx_newcontract(self):
        ...
        assert (
            hexlify(sig_r)
            == b"d043caa5f35e7676e0a9f9f0cd72a4055ae715309e8db8b3b6f1db928d276222"
        )
        assert (
            hexlify(sig_s)
            == b"08b91becc7c4ffbea18fe499fb3289bc7384880c2f7d86f4e831191a286553f6"
        )
